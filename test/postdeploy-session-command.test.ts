...
test("session command preserves archive then database checks with bounded requests and database operations", async (t) => {
  ...
  t.mock.method(globalThis, "fetch", async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (path.includes("/v1/internal/auth/users/")) {
      events.push("session-version");
      return Response.json({ sessionVersion: 1 });
    }
    if (path === "/v1/turns") {
      events.push("model");
  ...
  assert.deepEqual(timeouts, [30_000, 300_000, 60_000, 30_000, 30_000, 30_000]);
