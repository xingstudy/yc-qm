...
test("production promotion is conflict-safe and retries the same digest idempotently", () => {
  ...
  assert.match(workflow, /QM_RELEASE_TAG=\.\*\/QM_RELEASE_TAG=\$RELEASE_TAG/);
  assert.match(workflow, /grep -qx 'QM_RELEASE_TAG=prod-v0\.0\.0'/);
  assert.match(workflow, /grep -qx "QM_RELEASE_TAG=\$RELEASE_TAG" default\.env\.production\.example/);
  assert.match(workflow, /default\.env\.production\.example/);
  assert.doesNotMatch(workflow, /"\.env\.production\.example#/);
  assert.match(workflow, /cp scripts\/init-production-env\.sh init-production-env\.sh/);
  assert.match(workflow, /cp scripts\/deploy-production-release\.sh deploy-production-release\.sh/);
  assert.match(workflow, /"deploy-production-release\.sh#Verified production release deployer"/);
  assert.match(workflow, /"release\.production\.tag#Signed production release identity"/);
  assert.match(workflow, /printf '%s\\n' "\$RELEASE_TAG" > release\.production\.tag/);
  ...
  assert.match(workflow, /for file in compose\.production\.yaml default\.env\.production\.example/);
