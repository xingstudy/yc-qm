...
test("service credentials use active organization users, units, and access groups as grantees", async () => {
  const srv = start();
  try {
    await srv.built.deploymentLayerReady;
...
test("a grantee outside the organization subject model is rejected", async () => {
  ...
  try {
    ...
    assert.equal(r.status, 400);
    assert.deepEqual((await getCfg(srv.base)).serviceCredentials[0]!.grantees.sort(), ["org:default-org"]);
  } finally {
    await srv.close();
  }
});
...
test("legacy channel credential grants do not bypass organization subject authorization", async () => {
  const { built, env } = await buildWithCapture();
  await built.serviceCreds.setServiceCredential("org:default-org", {
    slug: "x-firehose",
    name: "X",
  ...
  assert.equal(res.status, "ok", res.reason);
  const token = env()?.AGENT_CREDENTIAL_TOKEN;
  assert.equal(token, undefined, "a legacy channel grant is not an organization access subject");
