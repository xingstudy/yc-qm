...
async function start() {
  ...
  await Promise.all(
    ["admin-alice", "admin-bob"].map((principalId) =>
      built.organization.backfillUser({ principalId, email: null, displayName: principalId }),
    ),
  );
...
test("an org admin promotes a user to org_admin; whoami reflects it; audited", async () => {
  const s = await start();
  try {
    await s.built.organization.backfillUser({ principalId: "U1", email: null, displayName: "U1" });
...
test("a cased stored grant is revoked by its canonical id (the delete follows the person, not the bytes)", async () => {
  const s = await start();
  try {
    await s.built.organization.backfillUser({
      principalId: "Jordan@Acme.test",
...
test("a suspended grant holder does not satisfy the last active admin guard", async () => {
  const { createAdminService } = await import("../src/admin/admin-service.ts");
  const { createAdminGrantStore, createMemoryAdminGrantPersistence } =
    await import("../src/admin/admin-grant-store.ts");
  const store = createAdminGrantStore(createMemoryAdminGrantPersistence(), {
    seed: [
      { principalId: "active-admin", scopeId: "org:default-org", role: "org_admin" },
      { principalId: "suspended-admin", scopeId: "org:default-org", role: "org_admin" },
    ],
  });
  const service = createAdminService(store, {
    isActivePrincipal: async (principalId) => principalId === "active-admin",
  });
  assert.deepEqual(await service.adminStatusOf({ id: "suspended-admin", type: "internal" }), { isAdmin: false });
  assert.equal(await service.canAdminister({ id: "suspended-admin", type: "internal" }, "org:default-org"), false);
  await assert.rejects(
    service.revokeGrant({ id: "active-admin", type: "internal" }, "active-admin", "org:default-org", "org_admin"),
    /cannot revoke the last org admin/,
  );
...
test("without DATABASE_URL grants are in-memory: the seed re-applies each boot; a runtime promotion is NOT durable", async () => {
  ...
  const b1 = await boot();
  try {
    await b1.built.organization.backfillUser({ principalId: "U-durable", email: null, displayName: "U-durable" });
    assert.equal(
