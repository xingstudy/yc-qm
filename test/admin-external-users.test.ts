...
import { adminStatusFromGrants, createAdminService } from "../src/admin/admin-service.ts";
...
function start(
  ...
  built.admin = createAdminService();
  const deps = {
    admin: built.admin,
...
test("an address that already belongs to an org member cannot be invited", async () => {
  ...
  try {
    ...
    await s.built.organization.backfillUser({
      principalId: "seen@elsewhere.example",
      email: "seen@elsewhere.example",
      displayName: "Seen",
    });
...
test("revoking the last org admin is refused and leaves the external record untouched", async () => {
  ...
  try {
    ...
    assert.match(((await r.json()) as any).message, /another.*admin/i);
