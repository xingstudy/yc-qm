...
test("configuration-dependent maintenance remains local while migration definitions remain immutable", async () => {
  const url = "postgres://does-not-exist:0/maintenance-local";
  const first = createPgPool(
    url,
    "test/local-maintenance/0001",
    ["SELECT 1"],
    [{ id: "test/local-maintenance/runtime", statements: ["SELECT 'org1'"] }],
  );
  const second = createPgPool(
    url,
    "test/local-maintenance/0001",
    ["SELECT 1"],
    [{ id: "test/local-maintenance/runtime", statements: ["SELECT 'org2'"] }],
  );
  assert.throws(
    () => createPgPool(url, "test/local-maintenance/0001", ["SELECT 2"]),
    /registered twice with different checksums/,
  );
