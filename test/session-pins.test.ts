...
import { serverDeps, buildApp, type BuiltApp } from "../src/wiring.ts";
...
describe("conversation pins self-API", async () => {
  ...
  const capFor = async (actorId: string, threadRef?: string) =>
    mintCapabilityToken(
      {
        actorId,
        ...
        liveActor: true,
        sessionVersion: (await built.organization.checkRuntimeActive(actorId))?.sessionVersion,
        ...(threadRef ? { threadRef } : {}),
      },
  ...
  before(async () => {
    built = buildApp(testConfig({ orgBootstrapUsers: ["U1", "U2", "U9"], signingSecret: SECRET }));
    server = createServer(built.app, {
      ...serverDeps(testConfig({ signingSecret: SECRET }), built),
