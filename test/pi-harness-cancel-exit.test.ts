...
test(
  "a mid-prompt cancel takes the stopped exit: partial persisted replay-safe, checkpointed, attested",
  { timeout: 15_000 },
  async () => {
    ...
    globalThis.fetch = ((_url: string | URL | Request, init?: RequestInit) =>
      new Promise((_resolve, reject) => {
        const signal = init?.signal ?? (_url instanceof Request ? _url.signal : undefined);
        if (signal?.aborted) {
