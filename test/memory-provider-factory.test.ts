...
test("configured provider runs OAuth MCP recall and explicit capture end to end", async () => {
    ...
    providers: [
      {
        id: "knowledge",
        type: "mcp",
        url: "https://knowledge.internal:8080",
  ...
  assert.equal(calls[0]?.url, "https://knowledge.internal:8080/token");
  const toolCalls = calls.filter(
    (call) => !call.url.endsWith("/token") && JSON.parse(call.body).method === "tools/call",
  );
  assert.equal(toolCalls[0]?.authorization, "Bearer ro-token");
  assert.match(toolCalls[0]?.body ?? "", /search_knowledge/);
  assert.equal(toolCalls[1]?.authorization, "Bearer rw-token");
  assert.match(toolCalls[1]?.body ?? "", /write_knowledge/);
});
