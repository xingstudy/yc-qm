...
test("unknown builtin-provider model fails resolution and a live runtime selection request", async () => {
  ...
  const config = testConfig({ orgBootstrapUsers: ["admin-alice"], harness: "pi" });
...
test("qualified custom model IDs coexist with overlays and OpenRouter failure rebuilds local choices", async () => {
  const custom = {
    id: "gateway",
    name: "Gateway",
  ...
  assert.doesNotThrow(() => validateCustomProviderSpec(custom));
  fail = true;
...
test("live admin lifecycle is authorized, audited, immediately selectable and removed on delete", async () => {
  const config = testConfig({ orgBootstrapUsers: ["admin-alice"], harness: "pi", openaiApiKey: "local-test-key" });
  const built = buildApp(config, {
    modelCredentialFetch: async () => Response.json({ data: [] }),
    modelVerificationProbe: async () => {},
  });
  ...
  try {
    ...
    await built.organization.backfillUser({ principalId: "alice", email: null, displayName: "Alice" });
    ...
    await built.organization.backfillUser({ principalId: runtime.principalId, email: null, displayName: "Admin" });
