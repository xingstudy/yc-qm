...
import type { SkillAccessRepository } from "../src/authorization/skill-access-repository.ts";
...
async function fixture(t: TestContext, config: Partial<Config> = {}) {
  const built = buildApp(
    testConfig({ orgBootstrapUsers: ["U1", "U2", "U3", "U4"], memoryCapture: "off", seedSkills: false, ...config }),
  );
  await built.config.hydrate?.();
  await built.identity.hydrate();
  await built.deploymentLayerReady;
  await built.organization.checkActive("U4");
  t.after(async () => {
  ...
  const access = async (id: string, principals: string[]) => {
    const actor = { principalId: "U1", isAdmin: true };
    const repository = built.skills as SkillAccessRepository;
    const current = await repository.getAccess(id, actor);
    await repository.setAccess(id, actor, {
      mode: principals.length ? "restricted" : "home",
      subjects: principals.map((id) => ({ kind: "user" as const, id })),
  ...
  return { ...built, turn, skill, access, remove };
}
...
test("sharing e2e: explicitly authorized skill assets execute, update, and disappear after revocation", async (t) => {
  const b = await fixture(t);
...
test("sharing e2e: room skills are removed from an existing DM computer after membership revocation", async (t) => {
  ...
  await b.turn("hello", true);
  assert.match(await b.turn("!read skills/room-helper/SKILL.md"), /scripts\/value.py/);
...
test("sharing e2e: local skill name wins, then exposes the carried fallback when archived", async (t) => {
  const b = await fixture(t);
  const carried = await b.skill("personal:U1", "duplicate-helper", "CARRIED");
  await b.access(carried.id, ["U1", "U2", "U3"]);
...
for (const memoryRecall of ["off", "writable"] as const) {
  test(`sharing e2e: memory ${memoryRecall} does not disable carried files or skills`, async (t) => {
    ...
    const skill = await b.skill("personal:U1", "independent-helper", "SKILL_WITH_MEMORY_RESTRICTED");
    await b.access(skill.id, ["U1", "U2", "U3"]);
...
test("sharing e2e: private skills remain private when the room speaker changes", async (t) => {
  const b = await fixture(t);
  const skill = await b.skill("personal:U1", "speaker-helper", "SPEAKER_ONE");
  await b.turn("!read skills/speaker-helper/SKILL.md");
  assert.equal(await b.turn("!run python3 skills/speaker-helper/scripts/value.py"), "SPEAKER_ONE");
  for (const actor of ["U1", "U2", "U1"]) {
    assert.match(await b.turn("!read skills/speaker-helper/SKILL.md", true, actor), /no file/);
    assert.equal(
      await b.turn(
  ...
  for (const actor of ["U1", "U2"]) {
    await b.turn("!read skills/speaker-helper/SKILL.md", true, actor);
    assert.equal(await b.turn("!run python3 skills/speaker-helper/scripts/value.py", true, actor), "SPEAKER_ONE");
  }
});
...
test("sharing e2e: screening off preserves carried skills without model calls", async (t) => {
  ...
  const skill = await b.skill("personal:U1", "unscreened-helper", "SHARED_SKILL_OK");
  await b.access(skill.id, ["U1", "U2", "U3"]);
