...
test("standalone PostgreSQL maintenance scripts load DATABASE_CA_CERT trust", () => {
  assert.match(source("../scripts/backfill-session-tape.ts"), /openSessionStore\(\)/);
  assert.match(source("../scripts/lib/backfill-runner.ts"), /configurePgCaTrustFromEnv\(process\.env\)/);
