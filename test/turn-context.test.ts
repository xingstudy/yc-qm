...
test("skill discovery uses included scopes and rejects legacy ACL skill grants", async () => {
  ...
  removeMember();
  await (await resolveTurnContext(input)).listSkills();
  assert.deepEqual(seen[1], { scopes: ["personal:alice", "org:test"], grants: undefined });
});
