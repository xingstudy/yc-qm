...
import { cronIdOf, entryWithinTenure, transcriptEntries, windowedTranscript } from "../sessions/session-store.ts";
...
> {
  ...
  async function directoryActor(
    ...
    return {
      principalId,
      isAdmin:
        allowAdminElevation && (await deps.admin?.adminStatusOf({ id: resolved, type: "internal" }))?.isAdmin === true,
    };
  }
  ...
  async function cronSessionForViewer(sessionId: string, principalId: string) {
    await deps.identity.refresh();
    if (!deps.identity.isInternal(deps.identity.classify(principalId))) return null;
    if (deps.organization && (await deps.organization.checkRuntimeActive(principalId))?.status !== "active")
      return null;
    const session = await deps.sessions.get(sessionId);
    const cronId = cronIdOf(session?.threadRef);
    if (!session || !cronId) return null;
    const cron = await deps.crons.get(cronId);
    if (!cron || !(await canAdministerCron(app, cron, principalId))) return null;
    const runs = await deps.crons.firesByThreadRefs([session.threadRef]);
    const retained =
      runs.some((run) => run.cronId === cronId && run.sessionId === sessionId) ||
      cron.fireLog?.some((run) => run.sessionId === sessionId);
    return retained ? session : null;
  }
  ...
  return {
    ...
    async getSessionForViewer(sessionId, principalId, window) {
      ...
      const pins = await decoratedPins(pinRecords, visible, (seq) =>
        participantSession ? viewerStoredEntryAt(sessionId, principalId, seq) : storedEntryAt(sessionId, seq),
      );
      return {
        session,
        entries: w.entries,
    ...
    async getSessionEntryForViewer(sessionId, principalId, seq) {
      ...
      const entry = participantSession
        ? await viewerEntryAt(sessionId, principalId, seq)
        : await renderedEntryAt(sessionId, seq);
    ...
    async listScopeResources(principalId, scope) {
      ...
      const [page, allCrons, allWebhooks, allDeployments, visibleSkills] = await Promise.all([
