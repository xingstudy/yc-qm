...
function createCodexHarnessInstance(opts: CodexHarnessOptions, binding?: NativeProviderBinding) {
  ...
  const authStore: CodexAuthStore | undefined = binding
    ? undefined
  ...
  const runPrompt = async (turn: HarnessTurnInput, toolsEnabled = true): Promise<HarnessTurnResult> => {
    ...
    if (turn.runtime?.modelId && !modelSupportedByHarness(turn.runtime?.modelId, "codex")) {
      throw new NonRetryableTurnError(`Model ${turn.runtime?.modelId} is unavailable for codex`);
    }
    const chosenModel = turn.runtime?.modelId ?? resolveModelId(turn.scopeLabel);
    if (!binding && !turn.codexAuth) {
      const selected = await resolveNativeProvider(
        chosenModel,
        "codex",
        opts.resolveCustomProvider,
        oauthConfigured ? undefined : opts.resolveModelCredential,
      );
      if (closed) throw new NonRetryableTurnError("codex harness is closed");
      if (turn.cancel?.aborted) return { reply: "", stopped: true };
      if (selected) {
        ...
        try {
          return await child.runPrompt(turn, toolsEnabled);
        } finally {
    ...
    try {
      ...
      model = binding?.model.id ?? chosenModel;
    ...
    try {
      ...
      state = {
        ...
        taskResults: new Set(),
        model: chosenModel ?? selectedModel,
        modelCalls: 0,
