...
export function createClaudeHarness(opts: ClaudeHarnessOptions = {}): Harness {
  ...
  const runPrompt = async (turn: HarnessTurnInput, toolsEnabled = true): Promise<HarnessTurnResult> => {
    ...
    if (turn.runtime?.modelId && !modelSupportedByHarness(turn.runtime?.modelId, "claude")) {
      throw new NonRetryableTurnError(`Model ${turn.runtime?.modelId} is unavailable for claude`);
    }
    const model = turn.runtime?.modelId ?? resolveModelId(turn.scopeLabel);
    const authEnv = opts.authEnv ? await opts.authEnv() : undefined;
    const binding = turn.claudeOauthToken
      ? undefined
      : await resolveNativeProvider(
          model,
          "claude",
          opts.resolveCustomProvider,
          authEnv?.CLAUDE_CODE_OAUTH_TOKEN ? undefined : opts.resolveModelCredential,
        );
    if (closed) throw new NonRetryableTurnError("claude harness is closed");
    if (turn.cancel?.aborted) return { reply: "", stopped: true };
    ...
    const sdkQuery = query({
      ...
      options: {
        ...
        env: perUserClaudeEnv(
          claudeChildEnv(
            nativeProviderEnv("claude", authEnv ? { ...opts.env, ...authEnv } : (opts.env ?? {}), binding),
            jail,
          ),
