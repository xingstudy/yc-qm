...
import { createKeyedQueue, sleep, withTimeout } from "./util/async.ts";
...
export function buildApp(
  ...
  const migrationsReady = registrationsReady.promise.then(() =>
    config.databaseUrl ? migrateRegisteredPgSchemas(config.databaseUrl) : undefined,
  );
  ...
  const identity = createIdentityService(artifactMap<DeactivationRecord>("deactivated_principals"), {
    statusBacking: artifactMap<IdentityStatusRecord>("organization_identity_status"),
  ...
  void migrationsReady
    .then(() => configStore.hydrate?.())
    .catch((error) => console.error("[wiring] config hydration failed:", errMessage(error)));
  ...
  const organizationMemberJobs = config.databaseUrl
    ? createPostgresOrganizationMemberJobStore(config.databaseUrl, { autoStart: false })
    : undefined;
  const organizationRootReady = migrationsReady.then(() =>
    organizationStore.ensureOrgRoot({
      orgId: config.orgId,
      name: config.orgId,
      actor: "system:bootstrap",
      now: Date.now(),
  ...
  organizationReady = organizationRootReady
    .then(() => Promise.all([organization.hydrate(), identity.hydrate()]))
  ...
  void migrationsReady
    .then(refreshCustomProviders)
    .catch((e) => console.error("[wiring] custom provider hydration failed:", errMessage(e)));
  ...
  const adapters = new Map<HarnessId, Harness>([
    ...
    [
      ...
      createCodexHarness({
        ...
        modelId: orgBaseModelId,
        resolveCustomProvider: (id) => customProviders.resolveConnection(id),
        resolveModelCredential: (provider) => modelCredentials.resolve(provider),
  ...
  const portalLoginTransactions = config.databaseUrl
    ? createPostgresPortalLoginTransactionStore(config.databaseUrl, { autoStart: false })
    : createMemoryPortalLoginTransactionStore();
  ...
  const admin = createAdminService(adminGrantStore, {
    advisoryLock,
    trustedOidcAdminIssuer: config.trustedOidcAdminIssuer,
    isActivePrincipal: async (principalId) => {
      if ((await organization.checkRuntimeActive(principalId))?.status !== "active") return false;
      const user = await organization.getUser(principalId);
      if (!user?.externalMembership || !user.invitedEmail) return true;
      const member = await identity.readExternalMember(user.invitedEmail);
      return member?.role === "org_admin" && member.expiresAt > Date.now();
    },
  });
  ...
  const runtime: Runtime = {
    start() {
      organizationMemberJobs?.start?.();
      portalLoginTransactions.start?.();
...
export function serverDeps(
  ...
  const baseModelDefault =
    configuredModel ?? defaultModelForHarness(config.harness, undefined, baseModelProviders(config));
