...
`<AUTH_ISSUER>/wecom/callback`. When WeCom returns a usable member email, the
broker maps the scan back to that email identity; otherwise it signs the member
in with a stable `wecom:<corpId>:<userid>` principal.
