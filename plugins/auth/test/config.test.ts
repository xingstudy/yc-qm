...
test("directory provider credentials are owned by core rather than auth", () => {
  assert.equal(
    problemsFor({
      AUTH_WECOM_CORP_ID: "partial-core-owned-value",
      AUTH_WECOM_REDIRECT_URI: "http://core-owned.example.test/callback",
      CORE_SIGNING_SECRET: "a".repeat(48),
    }),
    "",
  );
