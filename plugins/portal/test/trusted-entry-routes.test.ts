...
const loginTransactions = new Set<string>();
...
const upstream = createServer((req, res) => {
  void (async () => {
    ...
    if (url.pathname === "/v1/auth/portal-login/create") {
      let body = "";
      for await (const chunk of req) body += chunk;
      const state = String(JSON.parse(body).state ?? "");
      const status = loginTransactions.has(state) ? "conflict" : "created";
      loginTransactions.add(state);
