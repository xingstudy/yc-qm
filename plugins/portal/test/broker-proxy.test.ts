...
test("the verify redirect is relayed back to the browser", async () => {
  ...
  assert.ok(redirect.headers.getSetCookie().every((value) => !value.startsWith("portal_oidc_handoff=")));
});
...
test("confirming a sign-in posts through, same-origin only", async () => {
  ...
  assert.ok(confirmed.headers.getSetCookie().some((value) => value.startsWith("qm_idp_session=")));
