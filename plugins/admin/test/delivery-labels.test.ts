...
test("tool-associated and standalone deliveries use the same surface labels", () => {
  assert.match(html, /dlabel.textContent = adminTr\(deliveryLabel\(delivery\)\)/);
  assert.match(html, /(?:localizedBadge|badge)\(deliverySurfaceLabel\(delivery\), "info"\)/);
  assert.match(html, /label.textContent = adminTr\(event.shadow \|\| isDelivery \? deliveryLabel\(event\)/);
  assert.match(html, /(?:localizedBadge|badge)\(deliverySurfaceLabel\(event\), "info"\)/);
  assert.doesNotMatch(html, /textContent = "Slack delivery"|const slackDelivery/);
});
