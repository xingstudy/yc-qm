...
const core = createServer((req: IncomingMessage, res) => {
  if ((req.url ?? "").startsWith("/v1/admin/whoami")) {
    res.writeHead(200, { "content-type": "application/json" });
    res.end(JSON.stringify({ isAdmin: req.headers["x-admin-actor"] !== "U-member@acme" }));
    return;
  }
  let body = "";
  ...
  req.on("end", () => {
    ...
    res.end(JSON.stringify(denied ? { error: "forbidden" } : { ok: true }));
  });
});
...
test("confirmation shows added/removed members, unique total, and returns the user's decision", async () => {
  const review = slice("      function governanceSaveReview(", '      document.querySelectorAll("[data-inherit]")');
