...
function visibleImProviderOptions(): typeof IM_PROVIDER_OPTIONS {
  const enabled = appState.me?.chatChannelProviders;
  return Array.isArray(enabled)
    ? IM_PROVIDER_OPTIONS.filter((option) => enabled.includes(option.id))
    : IM_PROVIDER_OPTIONS;
}
...
function imChannelIds(status: ImBindingRecord["status"]): ImProviderId[] {
  return visibleImProviderOptions()
    .map((option) => option.id)
    .filter((id) => imBindings[id]?.status === status);
}
      ...
      imProviderMenuOpen
        ? html`<div class="im-add-wrap">
            <div class="im-provider-menu" role="menu">
              ${visibleImProviderOptions().map((option) => {
...
export function renderSidebarTop(): void {
  ...
  const navRow = (v: View, glyph: IconNode, label: string) =>
    html`<a
      ...
      data-view=${v}
      aria-label=${t(label)}
      ${tip(sidebarOpen ? "" : t(label))}
    >
      ${icon(glyph, 17)}<span>${t(label)}</span>
    </a>`;
...
function sidebarToggleLabel(): string {
  return t(sidebarOpen ? "Hide sidebar" : "Show sidebar");
}
