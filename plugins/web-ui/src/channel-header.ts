...
export async function loadChannelHeader(scopeId: string, onChange: () => void): Promise<void> {
  ...
  try {
    ...
    if (seq !== loadSeq) return;
    const fallback = "Couldn't load the pinned header setting.";
    const message = errMessage(e, fallback);
    channelHeaderState.notice = message === fallback ? t(fallback) : message;
...
async function save(scope: string, on: boolean | null): Promise<void> {
  ...
  try {
    ...
    channelHeaderState.on = r.on;
    channelHeaderState.configured = r.configured ?? null;
    channelHeaderState.notice = t(r.on ? "Header pinned in the channel." : "Pinned header removed.");
    channelHeaderState.noticeKind = "saved";
  } catch (e) {
    if (seq !== loadSeq) return;
    const fallback = "Couldn't update the pinned header setting.";
    const message = errMessage(e, fallback);
    channelHeaderState.notice = message === fallback ? t(fallback) : message;
    channelHeaderState.noticeKind = "error";
  } finally {
          ...
          ? fieldSelect({
              ...
              options: [
                html`<option value="default" ?selected=${channelHeaderState.configured === null}>
                  ${t("Default")} (${t(channelHeaderState.orgDefault ? "On" : "Off")})
                </option>`,
