...
export function createChatSurface(
      ...
      <div class="bg-row watch">
        <div class="bg-row-head static">
          ...
          <span class="bg-row-meta"
            >${t("armed")} ${relTime(w.createdAt)} · ${watchActivityLabel(w)} · ${timeLeft(w.expiresAt)}</span
          >
        </div>
      </div>
    `;
        ...
        >
          ...
          <span class="live-work-label"
            >${
              summary
                ? t(summary.label)
                : sheenLabel(stopping ? t("Stopping…") : `${t("Thinking")}${usedToolsSuffix(work)}`, true)
            }</span
          >
  ...
  function workLabel(work: WorkBlock): string {
    ...
    if (currentTextPhase(work)?.phase === "final_answer") return t(workedLabel("Worked", workSeconds(work)));
  ...
  function workBlock(
    ...
    let label = stopping ? t("Stopping…") : workLabel(work);
    if (stopped) label = `${t("You stopped after")} ${goalElapsedLabel(0, workSeconds(work) * 1000)}`;
    let fold =
  ...
  function toolPayloadText(payload: ToolPayload, extraHidden: string[] = []): string {
    const hidden = new Set(["tool", "callId", "workStartedAt", "workFinishedAt", "isError", ...extraHidden]);
  ...
  function toolDisclosure(
    ...
    const hasExecOutput = execution && toolExecutionOutput(result) !== null;
    ...
    return html`<div class="tool-disclosure">
      ...
      ${hasExecOutput ? execOutputCard(result, work, row.result) : nothing}
  ...
  function toolRow(row: ToolRowModel, work: WorkBlock, status: WorkBlock["status"], stale = false): TemplateResult {
    ...
    const head = html`<span class="tool-icon">${icon(rowIcon, 15)}</span>
      ${session ? html`<span class="session-action">${t(session.label)}</span>${sessionView?.chipTitle ? subagentChip(sessionView.chipTitle, sessionView.sessionId) : nothing}${sessionDetail ? html`<span class="tool-label session-message" title=${sessionDetail}>${sessionDetail}</span>` : nothing}` : html`<span class="tool-label" title=${detail ? `${t(label)}: ${detail}` : t(label)}>${visible}</span>`}`;
  ...
  function execOutputCard(result: ToolPayload, work: WorkBlock, activity: ToolActivity | null): TemplateResult {
    ...
    return html`<div class="code-card">
      ...
      <div class="code-card-foot">
        ${t("exit")} ${result.code ?? "unknown"}${result.timedOut ? ` · ${t("timed out")}` : ""}
        ${
          activity?.truncated
