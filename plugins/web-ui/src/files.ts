...
import { contextsState, ensureContexts, personalScopeId, scopeChip, scopeTitle } from "./contexts";
  ...
  render(
    html`
      ...
      <button
        ...
        @drop=${onFileDrop}
      >
        ${icon(Upload, 16)}<span>${t(dropLabel)}${uploadTarget ? scopeChip(uploadTarget) : nothing}</span>
      </button>
      ${
        visible.length
          ...
          : html`<div class="empty compact">${t(filtered ? "No files match these filters." : "No files yet.")}</div>`
      }
