...
function permissionBadge(d: DeploymentView): TemplateResult {
  ...
  return html`<span class="deploy-permission ${manage ? "manage" : "view"}" ${tip(t(title))}
  ...
  const title = html`
    <span class="deploy-row-title">
      ...
      <span class="list-row-meta deploy-row-meta"
        >${ownerLabel(d)} ${permissionBadge(d)} ${versionLabel(d)} · ${deployedLabel(d)}</span
      >
    </span>
  `;
  return html`
    <div class="list-row deploy-row ${d.status === "archived" ? "deploy-row-archived" : ""}">
      ...
      <div class="deploy-row-actions" aria-label="App status and actions">
        ...
        <button
          ...
          type="button"
          aria-label=${t(`Manage ${deploymentTitle(d)}`)}
