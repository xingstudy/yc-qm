...
test("the sidebar row's archive button names itself on hover", () => {
  ...
  assert.match(archive, /\$\{tip\(t\(s\.archived \? "Unarchive" : "Archive"\)\)\}/);
...
test("row tooltips reuse the wording the chats page already uses for the same actions", () => {
  const chatsPage = block('${tip(t(s.archived ? "Unarchive" : "Archive"))}');
  assert.ok(chatsPage.length > 0);
  assert.equal(sessions.match(/\$\{tip\(t\(s\.archived \? "Unarchive" : "Archive"\)\)\}/g)?.length, 2);
