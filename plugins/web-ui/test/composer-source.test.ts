...
test("steering transfers the durable queued run and reconciles races", () => {
  assert.match(composer, /const outcome = await ctx\.chat\.signalLiveRun\("steer", queued\.text, queued\.runId\);/);
  assert.match(composer, /if \(outcome\.ok \|\| outcome\.replayed\)/);
  assert.match(composer, /outcome\.reason === "queued_changed"/);
  assert.match(composer, /outcome\.reason === "queued_started" \|\| outcome\.reason === "not_found"/);
  assert.match(composer, /ctx\.chat\.resumeIfIdle\(\);/);
});
