...
test("the footer channel button opens the IM channel popover", () => {
  ...
  assert.match(shell, /function visibleImProviderOptions\(\): typeof IM_PROVIDER_OPTIONS/);
  assert.match(shell, /appState\.me\?\.chatChannelProviders/);
  ...
  assert.match(shell, /visibleImProviderOptions\(\)[\s\S]*\.map\(\(option\) => option\.id\)/);
  ...
  assert.match(shell, /visibleImProviderOptions\(\)\.map\(\(option\) => \{/);
...
test("each IM platform uses its official authorization and message client", () => {
  ...
  assert.match(server, /provider === "wecom" \? "work-wechat" : provider/);
  assert.match(server, /chatChannelProviders: enabledImProviders\(\)/);
  assert.match(server, /chatChannelsEnabled: chatChannelsEnabled\(\)/);
  assert.match(server, /if \(chatChannelsEnabled\(\)\) \{/);
  assert.match(syncWeixinBridge, /if \(!imProviderEnabled\("wechat"\)\) return/);
  assert.match(syncImSdkBridges, /IM_SDK_PROVIDERS\.filter\(imProviderEnabled\)/);
  assert.match(server, /enabledImProviders\(\)\.map\(\(provider\) => listUiStateRecords/);
