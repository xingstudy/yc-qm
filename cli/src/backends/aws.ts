...
export function secretArns(config: QmConfig): Record<string, string> {
  const aws = requireAws(config);
  const entries = computedSecrets(config).flatMap<{ name: string; arn: string; value: string | undefined }>(
    (secret) => {
      ...
      try {
        if (!secret.required) {
          ...
          return metadata.ARN && !metadata.DeletedDate && hasCurrent
            ? [{ name: secret.name, arn: metadata.ARN, value: undefined }]
            : [];
        }
  ...
  if (aws.backgroundWorkControl && values.get("CORE_SIGNING_SECRET") === values.get("DEPLOYMENT_CONTROL_SECRET"))
...
export async function awsSecretsPush(config: QmConfig, configDir: string, envFile?: string): Promise<void> {
  ...
  try {
    ...
    await withAwsLease(aws, async () => {
      ...
      let candidates = workloads.filter((workload) =>
      ...
      if (aws.backgroundWorkControl && candidates.includes("core")) {
        note(
        ...
        candidates = candidates.filter((workload) => workload !== "core");
      }
