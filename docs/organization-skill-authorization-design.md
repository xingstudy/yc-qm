...
- 升级会使所有现存 Portal 会话失效；用户下次登录时按准入策略处理，`domain_auto_join` 下持有受信邮箱的用户自动加入。
- `invite_only` 部署必须先用 `ORG_BOOTSTRAP_USERS` 预置首个管理员，否则没有任何账号能完成首次登录。
- `OIDC_PRINCIPAL_CLAIM=sub` 的部署不被此流程支持：登录没有受信邮箱，无法匹配邀请也无法自动加入，必须使用 `OIDC_PRINCIPAL_CLAIM=email`。
...
升级时会一次性冻结仅供 Slack/core 基础 turn 和既有 cron 使用的旧 participant allowlist。该兼容路径不创建活跃组织成员，不授予 Portal、Capability、目录、Skill 或管理员权限；当前内部目录身份和未停用状态仍会在每次使用时校验。停用兼容用户会写入持久化的离职 tombstone，不能通过重新出现在外部目录中恢复。所有旧用户完成正式登录或管理员确认后，应删除 `organization_legacy_runtime_eligible` 中已迁移的记录；后续版本移除兼容判定和该表。
