...
本清单定义 `xingstudy/yc-qm` 吸收上游 v0.1.11 时必须保留的下游行为，以及升级新接口所需的验证。已完成分支上的实际 merge、语义适配、重点独立复审和本地 Compose 验证；详细结果见 [集成记录](./upstream-v0.1.11-integration.md)。未签收的外部平台及发布演练继续阻断对应上线范围。
...
用户于 2026-09-14 确认当前生产部署为 `prod-v1.3.0`，并确认企业微信测试租户、Bot/测试用户与模型供应商测试配置已具备。现已按用户指示复用本地 Compose 配置，经指定 HTTPS 地址完成页面和一轮真实模型调用；企业微信扫码与 Bot 外发尚未签收，凭据不进入本清单。已从 origin 拉取并核对 tag，实际提交为 `bd1cb20411a69ff7badd95da4d482961b07d4b7b`，与本清单下游基线相同。[生产 Release](https://github.com/xingstudy/yc-qm/releases/tag/prod-v1.3.0) 发布于 2026-09-10，当前列出 9 个附件；附件存在不替代后续签名/内容校验。
...
| SKL-04 / P0 | 陈旧 fingerprint、名称碰撞、第二项创建失败；首项创建后 kill；补偿 delete 失败；两个 PG 进程并发 confirm              | PG 路径在单次组织事务内提交资源、ACL 和审计；提交前异常或 kill 后无可见半成品，并发确认仅一批发布；内存 backing 不宣称崩溃原子                             | [skills HTTP](../test/skills-http.test.ts) 仅部分覆盖；kill/delete-failure/PG 两进程需补测                                                                                   |
...
| SKL-04a | PG 事务首项写入后 SIGKILL：另一进程 list/detail/resolve/policy/audit 不可见半成品；连接关闭后事务回滚，不删除同名已有资源。已有独立进程回归。                                            |
| SKL-04b | PG 批次后续写入或审计失败：同一事务整体回滚，不依赖逐项 delete 补偿；原资源/授权不变，不留下成功审计。提交成功但响应丢失后的重试允许返回名称冲突。                                       |
...
| GAP-13 | 基线缺陷 / SKL-02              | 已修复批量/逐文件物化 mode，增加缓存版本及 bundle 执行位 hash；真实文件系统 stat/执行、双向变更、失败无 marker 回归通过；真实 Docker 批量/逐文件执行、权限双向变更及二进制保留验证通过，见集成记录                                                                                                           | 启用 executable 技能资产的物化路径                 |
| GAP-14 | 新增恢复要求 / SKL-04          | 设计独立复审后采用现有 PG 单事务提交 Skill、ACL、revision、审计。异常和 SIGKILL 无可见半成品，PG 独立进程竞争仅一批成功；响应丢失重试可能冲突，内存路径不承诺崩溃原子                                                                                                                                        | 多技能导入的崩溃恢复验收；不是已有基线保证         |
...
| S2 集成准备  | 保存基线证据；从 origin 创建 worktree；按固定 tag 合并；记录冲突与无冲突调用者       | 逐模块设计决策完整，迁移/凭据/部署保留方案具体                                      | 实际 merge 完成，169 文件冲突已解 |
| S3 模块集成  | 数据库公共接口 → 身份权限 → 模型运行时 → 沙箱/交付 → 页面 → 部署                     | 相应行为与消费者测试、lint、项目 typecheck 通过；失败路径有覆盖                     | 见下文当前状态                    |
| S4 真实 QA   | 专用旧库升级；`dev-instance --no-slack` 页面、真实供应商及配置启用路径；生产形态回滚 | 所有首批启用 P0/P1 的 required 子项通过且 PG 0 skip；环境阻塞继续阻断；保存页面截图 | 见下文当前状态                    |
| S5 审查与 CI | 独立代码审查；向 origin 提交 PR；等替换 CI 完成                                      | 无未解决阻断发现；CI 完成而非仅排队；产品证据与最终 SHA 一致                        | 见下文当前状态                    |
| S6 发布      | 签名镜像/附件、升级及回滚检查；单独启用新架构                                        | 发布与部署分别记录；新开关/新拓扑不得绕过其门槛                                     | 见下文当前状态                    |

S1 文档审查通过只允许进入代码集成，不能替代 S3/S4。当前 S3 的完整包测试及失败文件复测、S4 的旧库副本升级/本地 Compose 页面/真实模型均已执行；S4 的外部平台和生产形态回滚尚未签收，S5 的重点独立代码复审通过、PR/CI 结果见集成记录；S6 未执行。GAP-13/14 的执行位修复和崩溃恢复是显式新增/修复要求，不声称基线已具备；已在 S2/S3 完成独立设计审查与实现，不能顺便扩大为通用存储重构。部署前仍须获得本次部署的明确授权；本清单不触发发布、发信或生产变更。
...
根目录的选定文件执行形式如下；具体文件按上面批次选取，保留所属脚本参数。示例仅用于说明调用形式；已执行的实际计数与证据以集成记录为准。
...
业务代码集成要 lint 改动代码、typecheck 所属 TS 项目并覆盖消费者；文档单独提交只需格式、相对链接、ID/状态完整性检查及实际消费本文的测试。设计阶段未跑全量业务套件；集成阶段因跨服务影响已执行完整包测试，完整 CI 仍为代码门槛。
...
| 将技能执行位/崩溃原子性当作已有保证        | GAP-13 记录实际 mode 丢失；GAP-14 明确崩溃隔离是新增要求；实施采用既有 PG 单事务，范围及限制见集成记录。 |
...
S1 当时实际检查：用 Linux Node 24.18.0 运行主工作区已安装 Prettier 对本文检查通过；85 个主用例（74 P0 / 11 P1）、30 个子项、15 个 GAP 的唯一性和引用关系检查通过；相对文件链接与 Markdown 表格结构检查通过；未发现已有测试读取本文路径，因此没有文档消费测试需要运行。没有执行应用构建、typecheck、PG、浏览器、真实模型、IM 外发或任何部署。该段是 S1 历史记录；当前执行证据与未签收项目见集成记录。

## S2 数据库前置进展（2026-09-14）

已完成四个下游存储的首批命名迁移兼容准备及独立复审，详细实现决策、固定 `prod-v1.3.0` fixture、独立审查结果与证据边界见 [PostgreSQL 前置报告](./upstream-v0.1.11-pg-preparation.md)。该报告记录 merge 前的准备阶段。目前实际 merge、全局注册迁移、事务池及整栈启动适配已完成；旧库副本重复升级与专用 PG 测试已执行。DB-01～DB-06 不因这些部分证据整体签收，尤其发布回滚及外部交付仍须独立验收。当前结果见 [集成记录](./upstream-v0.1.11-integration.md)。
